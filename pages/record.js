...
const myAccount = () => {
	...
	useEffect(() => {
		if (network === 'BSC Mainnet' || network === 'Polygon' || network === 'Ethereum' || network === 'Mumbai') {
			userRecord();
		}
	}, [currentAccount, network]);
            ...
            return (
                <Box sx={{ pt: 8, pb: 6 }}>
                    ...
                    <Typography variant="body1" align="center" color="text.secondary" paragraph>
						...
						<ColorButton align="center" variant="contained" onClick={switchToMumbai} style={{ fontFamily: "Unbounded, cursive"}}>
							Login With Arbtrium
						</ColorButton>
						<ColorButton align="center" variant="contained" onClick={switchToMumbai}  style={{ fontFamily: "Unbounded, cursive"}}>
							Login With Mumbai
					...
					<Box sx={{ display: 'inline', alignItems: 'center', '& > :not(style)': { p: 1, mt: 1 }, }}>
						...
						{(
							<Stack spacing={2} direction="row">
								<ColorButton  style={{ fontFamily: "Unbounded, cursive"}}   sx={{ width: '100%', mt: 4, mb: 4 }} variant="contained" onClick={userRecord}>
									Update Record
								</ColorButton>
							</Stack>
						)}
	...
	return (
		<>
			<AppBar position="static">
                <Toolbar>
				<Typography variant="body1" sx={{ flexGrow: 1, mt: 1 }} style={{ fontFamily: "'Audiowide', cursive",fontSize:"28px"}}>
						ARB 
					</Typography>
					<Chip
						avatar={<Avatar alt="Network logo" src={network.includes("BSC Mainnet") ? '/bsc-logo.svg' : network.includes("Polygon") ? '/polygon-logo.svg' :  network.includes("Mumbai") ? '/polygon-logo.svg' : network.includes("Ethereum") ? '/ethlogo.png' : '/ethlogo.png'} />}
