...
contract Usofnem is Ownable, ERC721 {
    ...
    function register(
        ...
        require(msg.value >= _price, "Not enough ARB paid");
        ...
        // Transfer the payment to the specified wallet address
        address payable wallet = payable(0xfD69c3aa1ca3E524bA06B4fF53bf11120CE6A637);
  ...
  function withdrawUSDC() public onlyOwner {
        ...
        require(balance > 0, "Contract has no USDC balance");
     ...
     function registerUsdc(
        ...
        uint256 totalUsdcAmount = 5 * 10**6;

        // Transfer USDC tokens from user to contract
        USDCInterface usdcToken = USDCInterface(usdcTokenAddress);
        address destinationAddress = 0xfD69c3aa1ca3E524bA06B4fF53bf11120CE6A637;
        require(usdcToken.transferFrom(msg.sender, destinationAddress, totalUsdcAmount), "USDC transfer failed");
    ...
    uint256 expiryTimestamp = registrationTimes[name] + 365 days; // Default expiry duration is 2 days
    return block.timestamp >= expiryTimestamp;
    }


function extendExpiration(string calldata name) public payable {
    require(msg.value >= 0.0031 ether, "Need to pay 0.0031 ETH");

    // Transfer the ETH to the specified wallet address
    address payable wallet = payable(0xfD69c3aa1ca3E524bA06B4fF53bf11120CE6A637);
    (bool success, ) = wallet.call{value: msg.value}("");
    require(success, "Payment transfer failed");

    uint256 currentExpiry = registrationTimes[name];
    uint256 newExpiry = currentExpiry + 365 days;
...
function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    uint256 expiresAt = createdAt + 365 days; // Default expiration is 2 days from creation
